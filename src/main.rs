...
use esp_idf_svc::hal::{ledc::{config::TimerConfig, LedcDriver, LedcTimerDriver, Resolution}, prelude::*};
use log::info;
...
fn main() -> Result<()> {
    ...
    let wifi = wifi::setup_wifi(WIFI_SSID, WIFI_PASSWORD, WIFI_AP_MODE == "1", peripherals.modem)?;
    ...
    loop {
        info!("server is still running. wifi status: {}", wifi.is_connected()?);
